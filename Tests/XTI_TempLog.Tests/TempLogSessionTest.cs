...
{
    public sealed class TempLogSessionTest
    {
        [Test]
        ...
        private TempLogSession getTempLogSession(IServiceProvider sp)
            => sp.GetService<TempLogSession>();

        private FakeTempLog getTempLog(IServiceProvider sp)
            => (FakeTempLog)sp.GetService<TempLog>();

        private FakeAppEnvironmentContext getAppEnvironmentContext(IServiceProvider sp)
            => (FakeAppEnvironmentContext)sp.GetService<IAppEnvironmentContext>();

        private FakeClock getClock(IServiceProvider sp) => (FakeClock)sp.GetService<Clock>();
    }
}

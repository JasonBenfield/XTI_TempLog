...
internal sealed class TempLogSessionV1Test
{
    ...
    private TempLogSessionV1 getTempLogSession(IServiceProvider sp)
        => sp.GetRequiredService<TempLogSessionV1>();

    private FakeTempLogV1 getTempLog(IServiceProvider sp)
        => (FakeTempLogV1)sp.GetRequiredService<TempLogV1>();
